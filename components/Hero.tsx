...
export const Hero: React.FC = () => {
  ...
  const roles = language === 'pl' 
    ? ['Full-Stack Developer', 'SaaS Founder', 'Tech Enthusiast'] 
    : ['Full-Stack Developer', 'SaaS Founder', 'Tech Enthusiast'];
    ...
    <section id="about" className="min-h-screen flex items-center pt-20 relative scroll-mt-20">
      ...
      <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-12 items-center relative z-10">
        ...
        >
          ...
          >
            ...
            <ul className="space-y-4 relative z-10">
              {reasons.map((reason, idx) => (
                ...
                >
                  <CheckCircle2 className="text-emerald-500/80 shrink-0 mt-1" size={18} />
                  <span className="text-neutral-300 text-sm">{reason}</span>
                </motion.li>
              ))}
            </ul>
          </motion.div>
          ...
          <div className="absolute -z-10 top-12 -right-8 w-full h-full border border-neutral-800/30 rounded-xl bg-neutral-900/10 blur-sm" />
        </motion.div>
      </div>
    </section>
  );
